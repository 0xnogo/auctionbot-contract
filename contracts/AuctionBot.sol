    ...
    UUPSUpgradeable
{
        ...
        )
    {
        ...
        }
        {
            ...
            sumAuctioningTokenAmount = sumAuctioningTokenAmount.sub(
                referralFee
            );
    ...
    function registerReferralEarnings(
        ...
        if (bytes(referralCode).length > 0) {
            referralFee = buyAmount.mul(referralFeeNumerator[auctionId]).div(
                FEE_DENOMINATOR
            );
            uint64 userId = getUserId(address(referralRewardManager));
            sendOutTokens(auctionId, referralFee, 0, userId);
